...
class Melolo : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        val ep = tryParseJson<EpisodeData>(data)
            ?: return false
        ...
        // DEBUG
        println("Melolo Response = $responseText")

        val resp =
            tryParseJson<PlayerVideoModelResponse>(responseText)
    ...
    override suspend fun getMainPage(
        ...
        val isSearchCategory =
            request.data.startsWith("q:", true)

        val books = if (isSearchCategory) {

            val url =
                "$catalogBase/search?query=${
                    URLEncoder.encode(
                        request.data.removePrefix("q:"),
                        "UTF-8"
                    )
                }&limit=20&offset=${(page - 1) * 20}"

            tryParseJson<CatalogSearchResponse>(
                app.get(url).text
            )?.data
                ?.search_data
                ?.flatMap { it.books }
    ...
    private suspend fun fetchLatest(): List<CatalogBook> {
        ...
        return tryParseJson<CatalogLatestResponse>(
            app.get("$catalogBase/latest").text
        )?.books
            .orEmpty()
            .filter {
                it.language.equals("id", true)
            }
    }

    private suspend fun fetchTrending(): List<CatalogBook> {

        return tryParseJson<CatalogTrendingResponse>(
            app.get("$catalogBase/trending").text
        )?.books
            .orEmpty()
            .filter {
                it.language.equals("id", true)
    ...
    private suspend fun fetchSearch(
        ...
        return tryParseJson<CatalogSearchResponse>(
            app.get(url).text
        )?.data
            ?.search_data
            ?.flatMap { it.books }
    ...
    private suspend fun fetchDetail(
        ...
        return tryParseJson<CatalogDetailResponse>(
            app.get("$catalogBase/detail/$bookId").text
        )?.data
            ?.video_data
            ?: throw ErrorLoadingException("Detail Empty")
    }
    ...
    data class PlayerVideoModelData(
        ...
        val video_info: PlayerVideoInfo? = null,

        @JsonProperty("video_model")
        val video_model: PlayerVideoModel? = null
    )
    ...
    data class PlayerVideoModel(
        ...
        val video_list: Map<String, PlayerVideoInfo>? = null
    )
    ...
    data class CatalogEpisode(
        ...
        @JsonProperty("vid_index")
        val vid_index: Int? = null,

        @JsonProperty("cover")
        val cover: String? = null,

        @JsonProperty("disable_play")
    ...
    data class EpisodeData(
        ...
        @JsonProperty("videoPlatform")
        val videoPlatform: Int
    )
}